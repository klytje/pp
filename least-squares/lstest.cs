...
public class main {
	public static void Main(string[] args) {
		...
		Func<double, double>[] funcs = new Func<double, double>[]{t => 1, t => t}; // not sure about this one
		var fit = ls.qrfit(x, y, dy, funcs);
		double lambda = fit.p[1]; // not sure about this one either
		double dlambda = Sqrt(fit.cov[1, 1]);
		
		int N = 100; // number of points for the model
		double step = (x[x.size-1] - x[0])/N;
		WriteLine($"# Evaluating fit with parameters: lambda: {lambda:F3}, dlambda: {dlambda:F3}");
